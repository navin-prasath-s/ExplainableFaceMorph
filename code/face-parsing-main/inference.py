...
def inference(params: argparse.Namespace) -> None:
    """
    Run inference on images using the face parsing model and save simple face cutouts.
    ...
    num_classes = 19  # Number of face parsing classes (default BiSeNet setting)
    ...
    for file_path in tqdm(files_to_process, desc="Processing images"):
        ...
        try:
            ...
            # Run inference
            output = model(image_batch)[0]  # feat_out, feat_out16, feat_out32 -> use feat_out for inference
            predicted_mask = output.squeeze(0).cpu().numpy().argmax(0)
            ...
            # --- Main change: Face Cutout ---
            # Create binary mask where face is present
            face_mask = (predicted_mask == 2).astype(np.uint8)  # Check if 1 is the correct face class

            # Apply the mask to the original image
            image_np = np.array(image)
            face_cutout = cv2.bitwise_and(image_np, image_np, mask=face_mask)

            # Create RGBA image (with transparent background)
            b_channel, g_channel, r_channel = cv2.split(face_cutout)
            alpha_channel = (face_mask * 255).astype(np.uint8)
            rgba = cv2.merge((b_channel, g_channel, r_channel, alpha_channel))

            # Adjust save path to PNG
